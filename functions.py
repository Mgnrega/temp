...
def get_encodings(image):
    ...
    try:
        ...
        if(len(encode) > 1):
            ...
            return return_json(0 , 3 ,'More than one faces in image' )
        else:
            encode = encode[0]
            encode = encode.tolist()
            return return_json(encode , 1, 'Attendence Taaken')
    except:
        return return_json(0 , 2 , "No Faces detected in image")
...
def recognize_image(image, gid):
    ...
    try:     
        ...
        cv2.waitKey(0)
        return return_json(attendance , 1, 'Made Attendance List') 
...
def mark_attendence():
    ...
    return return_json(0 ,  1 , "Marked attendence" )
